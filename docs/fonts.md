...
- **The build ships no fonts.** They arrive as downloadable **font packs** (see below) or as files the user drops in by hand. `ScanFonts(modPath)` walks `<mod>/Fonts/` and `<mod>/Resources/` **recursively** (packs install one folder deep) and turns every `.ttf`/`.otf` into a `FontEntry`; the file name minus extension is the entry name, so pack files must keep the names settings save. First name seen wins. Then `LinkFamilies` groups by family and wires each entry's bold sibling (exact `Bold`, else the heaviest ≥ Bold). `DropStaleBundle` deletes the pre-1.4 `Resources/bismuth-fonts` AssetBundle on the way past — updates extract over an install and never delete, so the 5.6 MB orphan would otherwise sit there forever.
- **An empty font list is the default state**, not an error: `UICore.ResolveSavedFont` and `MainClass.ApplySelectedFont` fall back to `GameFontApplier.GameFont` — the last font the game itself stamped through `RDString.SetLocalizedFont`, or `TMP_Settings.defaultFontAsset` before one is seen. TMP's own default is Latin-only, which tofus a Korean panel; the game's is whatever that language actually renders with.
- `FontEntry.TmpFont` lazily builds a dynamic-SDF `TMP_FontAsset`, naming it `"<name> (TMP)"` and wiring the family's real Bold into `fontWeightTable[7]` so `<b>`/`FontStyles.Bold` render true bold glyphs. It builds from Unity 6's `CreateFontAsset(filePath, …)`, which stores the path and reloads the face on demand (`AtlasPopulationMode.Dynamic`) — so Korean/CJK glyphs rasterize from the file as needed and persist across scenes.
...
**Defaults (Settings.cs):** overlay font `FontName` and panel/game fonts (`UiFontName`/`GameFontName`) default to **`Paperlogy-4Regular`**. That name resolves to nothing until the Paperlogy pack is installed — the fallback above covers the gap, and installing the pack makes the defaults take effect. Paperlogy carries CJK glyphs, so Japanese/Korean level titles render without falling back. (`FontName` was 에이투지체 until 1.2.x; 에이투지체 lacks kana, which drew Japanese titles undersized via the global fallback.)

### Font packs (`Util/FontPacks.cs`, `UI/Pages/PageFontPacks.cs`)

Appearance → **Font packs** lists packs from a manifest (`fonts.json` at the repo root, served raw from GitHub — a plain file, not the releases API, which is rate-limited per IP). Each entry is `{Id, Name, Note, Size, Url}`; `Size`/`Note` are display strings straight from the manifest, never computed client-side. The zips live in the repo too, under `font-packs/`, and are fetched raw from `main`: the releases list then holds mod builds and nothing else, so nobody hunting for Bismuth trips over a font tag. The cost is that git keeps every revision of a binary forever — rebuild a pack only when its fonts actually change.

- Install downloads through `UpdateChecker.FetchBytes` (curl first, `WebClient` fallback — both `UnityWebRequest` and a bare `WebClient` have hung silently under MelonLoader) on the thread pool, drained by `FontPacks.Tick()` from `MainClass.OnUpdate`, then extracts `.ttf`/`.otf`/`.txt` **flat** into `Fonts/<Id>/`. Flattening is also the traversal guard: no entry path from the zip is ever joined onto the destination.
- Install/remove finish with `MainClass.RequestForceReload()` — the same rescan+rebuild+reapply as Misc → Debug → Force reload. It disposes the panel, so it's deferred to the next frame, and the page pushes with `rebuildOnReveal`.
- One install at a time (`Busy`): two unzips into `Fonts/` would both fire a rescan mid-write.
- Remove is `Directory.Delete(Fonts/<Id>, true)`. The saved font name is left alone — every resolver falls back, so reinstalling restores the choice.
- `tools/build-font-packs.sh <font-dir>` builds the zips **and** the manifest from a folder of correctly-named font files, so the two can't drift.
...
- **Material restore (TMP):** `TmpState.Mat` caches `fontSharedMaterial` and Restore re-assigns it AFTER the font (set_font resets to the font asset's default material). The game styles TMP per-text via `fontMaterial` instances — `RDString.SetLocalizedFont` explicitly carries `_UnderlayColor`/`_UnderlayDilate` across font stamps — so restoring the font alone gave e.g. the title-screen news sign the default material's dark outline. The re-stamp adoption path updates `Mat` together with `Font`.
...
- **Other mods' UI:** `Skip()` leaves foreign mod hierarchies alone via two signals: `IsForeignModUi` (any ancestor MonoBehaviour defined in a non-game, non-engine assembly — catches component-marked mod HUDs like TUFHelper's), and `IsOtherModRoot` (transform root named after another loaded mod's Id, from `UnityModManager.modEntries`, ≥4 chars, refreshed each full sweep). The root-name check exists because some foreign labels are plain TMP directly under a bare canvas — e.g. Sapphire's toolbar tooltip and timeline clock/BPM carry **no** Sapphire component anywhere in their chain, invisible to the assembly walk; but every Sapphire hierarchy roots at a `Sapphire…`-named object. Falls back to `{"Sapphire"}` if the registry is unreadable.
